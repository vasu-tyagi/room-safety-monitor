...
- [~] **Slice 5 — Real VLM.** Qwen 2.5 VL 7B via HF Inference Providers. Stub fallback via VLM_MODE env var (real/auto/stub). `{kb_context}` placeholder ready for Slice 6 KB retrieval. Evidence clips written to MinIO deferred (MinIO wired but clip bundling not yet built).
